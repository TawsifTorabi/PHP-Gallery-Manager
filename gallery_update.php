    ...
    if (!empty($_FILES['media']['name'][0]) && empty($errors)) {
        foreach ($_FILES['media']['tmp_name'] as $key => $file_tmp) {
            ...
            if ($php_error !== UPLOAD_ERR_OK) {
                ...
                switch ($php_error) {
                    case UPLOAD_ERR_INI_SIZE:
                        $errors[] = "[$file_name] exceeds upload_max_filesize.";
                        break;
                    case UPLOAD_ERR_FORM_SIZE:
                        $errors[] = "[$file_name] exceeds form limit.";
                        break;
                    case UPLOAD_ERR_PARTIAL:
                        $errors[] = "[$file_name] was only partially uploaded.";
                        break;
                    case UPLOAD_ERR_NO_TMP_DIR:
                        $errors[] = "Server missing temporary folder.";
                        break;
                    default:
                        $errors[] = "[$file_name] failed with error code: $php_error";
                }
                continue;
            }
            ...
            if (move_uploaded_file($file_tmp, $target_path)) {
                // 1. IMAGE PROCESSING (WebP to JPG)
                // Do this BEFORE database insertion so we save the correct filename (.jpg)
                if ($file_type === 'image/webp') {
                    ...
                    if ($image) {
                        ...
                        if (imagejpeg($bg, $jpg_path, 80)) {
                            unlink($target_path); // Delete original webp
                ...
                // 3. SINGLE DATABASE INSERTION
                // Make sure your table has the 'status' column!
                ...
                if ($stmt->execute()) {
                    ...
                    // 4. TRIGGER BACKGROUND COMPRESSION (Only for Videos)
                    if ($media_type === 'video') {
                        // Detach process using > /dev/null 2>&1 &
                ...
                $errors[] = "Failed to move [$original_name] to uploads directory.";
            }
        }
    }
